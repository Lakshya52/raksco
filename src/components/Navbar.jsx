      ...
      <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-305 ${scrolled ? 'bg-white/95 backdrop-blur-md shadow-md py-2.5 border-b border-slate-100' : 'bg-white/95 backdrop-blur-md py-4'
        ...
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          ...
          <nav className="hidden md:flex gap-8 items-center">
            ...
            <Link
              to="/"
              className={`nav-link relative py-1.5 transition-colors ${isActive('/') ? 'text-accent font-semibold' : 'text-slate-700 hover:text-accent'
                }`}
            >
              Home
              {isActive('/') && (
            ...
            <Link
              to="/about"
              className={`nav-link relative py-1.5 transition-colors ${isActive('/about') ? 'text-accent font-semibold' : 'text-slate-700 hover:text-accent'
                }`}
            >
              About Us
              {isActive('/about') && (
            ...
            <Link
              to="/leadership"
              className={`nav-link relative py-1.5 transition-colors ${isActive('/leadership') ? 'text-accent font-semibold' : 'text-slate-700 hover:text-accent'
                }`}
            >
              Leadership
              {isActive('/leadership') && (
            ...
            <div className="relative group py-1.5">
              <Link
                to="/services"
                className={`nav-link relative py-1.5 flex items-center gap-1 transition-colors ${isActive('/services') ? 'text-accent font-semibold' : 'text-slate-700 hover:text-accent'
                  }`}
              >
                Services
        ...
        {mobileMenuOpen && (
          <div className="md:hidden absolute top-full left-0 w-full bg-white shadow-xl border-t border-slate-100 py-6 px-6 flex flex-col gap-4 max-h-[85vh] overflow-y-auto" data-lenis-prevent>
            ...
            <Link
              to="/"
              className={`nav-link block text-lg font-medium p-2 rounded-xl transition-colors ${isActive('/') ? 'text-accent bg-accent/5 font-semibold' : 'text-slate-800'
                }`}
            >
              Home
            </Link>
            ...
            <Link
              to="/about"
              className={`nav-link block text-lg font-medium p-2 rounded-xl transition-colors ${isActive('/about') ? 'text-accent bg-accent/5 font-semibold' : 'text-slate-800'
                }`}
            >
              About Us
            </Link>
            ...
            <Link
              to="/leadership"
              className={`nav-link block text-lg font-medium p-2 rounded-xl transition-colors ${isActive('/leadership') ? 'text-accent bg-accent/5 font-semibold' : 'text-slate-800'
                }`}
            >
              Leadership
            </Link>
            ...
            <div className="rounded-xl border border-slate-150">
              <button
                ...
                className={`w-full text-left text-lg font-medium p-4 flex justify-between items-center transition-colors rounded-xl ${isActive('/services') ? 'text-accent bg-accent/5 font-semibold' : 'text-slate-800 bg-slate-50/50'
                  }`}
              >
                Services
                <ChevronDown size={18} className={`transition-transform duration-300 ${mobileServicesOpen ? 'rotate-180 text-accent' : 'text-slate-450'}`} />
              </button>
